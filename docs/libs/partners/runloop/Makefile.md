# `libs/partners/runloop/Makefile`

## Purpose

Development workflow automation for the `langchain-runloop` package. Provides standardized targets for testing, linting, type checking, and formatting using `uv` with frozen dependencies. Identical in structure to the `modal` and `quickjs` partner Makefiles, differing only in the package name (`langchain_runloop`).
...
| `lint_package` | Lint only the `langchain_runloop` package directory |
| `type` / `typecheck` | Run `ty check langchain_runloop` |
